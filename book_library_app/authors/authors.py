...
def get_authors():
    ...
    query = apply_order(query, Author)
...
def update_author(user_id: int, args: dict, author_id: int):
    author = Author.query.get_or_404(author_id, description=f'Author with id: {author_id} not found')
    author.first_name = args.get('first_name')
    author.last_name = args.get('last_name')
    author.birth_date = args.get('birth_date')
...
def delete_author(user_id: int, author_id: int):
    ...
    return jsonify({
        'success': True,
        'data': f'Author with id: {author_id} deleted'
    })
